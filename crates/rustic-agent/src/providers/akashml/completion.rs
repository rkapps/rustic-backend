...
#[derive(Debug)]
pub struct AkashMLClient {
    inner: Box<dyn LlmClient>,
}

impl AkashMLClient {
    /// Create a `AkashMLClient` that speaks the OpenAI HTTP API against `base_url`.
    pub fn new(api_key: String) -> Result<AkashMLClient> {
        info!(
            target: "agent-akashml",
            "AkashML request"
        );

        Ok(Self {
            inner: Box::new(OpenAIClient::new_with_chat_completions(
                AKASHML_BASE_URL.to_string(),
                api_key,
            )?),
        })
    }
...
impl LlmClient for AkashMLClient {
    ...
    async fn complete_with_stream(
        ...
        self.inner
            .complete_with_stream(akashml_request(request))
            .await
    }
}

pub fn akashml_request(request: CompletionRequest) -> CompletionRequest {
    CompletionRequest {
        id: request.id.clone(),
